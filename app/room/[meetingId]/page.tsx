...
export default function RoomPage({ params }: { params: Promise<{ meetingId: string }> }) {
  ...
  const [requiresGuestLogin, setRequiresGuestLogin] = useState(false)
      ...
      <div className="flex min-h-dvh items-center justify-center bg-black px-4">
        <div className="w-full max-w-md space-y-6 rounded-2xl border border-zinc-800 bg-zinc-900 p-8 shadow-2xl">
          ...
          <form onSubmit={(e) => {
            ...
            const identity = `guest_${Math.random().toString(36).substring(2, 11)}@guest.local`;
          ...
          <div className="text-center">
            <p className="text-sm text-zinc-400">
              Already have an account? <Link href={`/login?redirect=/room/${meetingId}`} className="text-indigo-400 hover:text-indigo-300">Log in</Link>
            </p>
          </div>
        </div>
      </div>
